...
export const selectNonDisabledUsers = createSelector([getUsers], users => {
  return users !== undefined
    ? users
        ...
        .filter(
          user =>
            user.platformStatus !== 'disabled' && user.status !== 'disabled'
        )
        .map(user => ({
          ...user,
...
export const filterUsersByPermissions = (state, users, permissionNames) => {
  ...
  return users.filter(
    user =>
      allRoles[
        allRoles.findIndex(role => role.id === user.roleId)
      ].permissions.filter(perm => permissionIds.includes(perm)).length ===
...
export const getUserTenantStatus = state =>
  getCurrentForm(state) && getCurrentForm(state).getIn(['values', 'status']);

export const getInvitationScenario = state => {
  const id =
    getCurrentForm(state) && getCurrentForm(state).getIn(['values', 'id']);
  const invitationStatus =
    getCurrentForm(state) &&
    getCurrentForm(state).getIn(['values', 'invitationStatus']);
  if (!id) {
...
*/
export const selectAvailableEntityMembersForList = (
  state,
  permissions = []
) => {
  let usersMap = null;
  ...
  if (Array.isArray(permissions) && permissions.length > 0) {
    ...
    usersMap = filterUsersByPermissions(
      state,
      availableEntityMembersForList(state),
      permissions
    );
  } else {
